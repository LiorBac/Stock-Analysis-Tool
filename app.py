...
if ticker:
    ...
    if hist.empty:
        ...
        if len(hist) >= 2:
            delta_val = hist['Close'][-1] - hist['Close'][-2]
            delta_str = f"{delta_val:.2f} since last close"
        else:
            delta_str = "N/A"

        col1,col2,col3 = st.columns(3)
        col1.metric("Current Price", f"${hist['Close'][-1]:.2f}", delta_str)
        col2.metric("Highest today", f"${hist['High'][-1]:.2f}")
        col3.metric("Lowest today", f"${hist['Low'][-1]:.2f}")

        st.markdown("---")

        st.subheader(f"Price Chart - {chart_type}")
